...
aggregate(data=data, ??????~???, mean)
aggregate(data=data, cbind(??????,??????,??????)~???, mean)
...
# 1
km2 = data[data$??? == '???' & data$?????? == '???' & data$?????? > 90 & data$?????? > 90, ]
km2[order(km2$??????),]

# 2
data$?????? = (data[, 4] + data[, 5] + data[, 6] + data[, 7] + data[, 8])/5
head(data)
aggregate(data=data[data$?????? > = 80], cbind(??????, ??????)~???, mean)


library(ggplot2)
install.packages("ggplot2")
data$pass = ifelse(data$?????? >= 60, TRUE, FALSE)
head(data)
data[data$pass, ]
data$scout = ifelse(data$?????? >= 60, 
                    ifelse(data$?????? == '???', 'BoyScout', 'GirlScout'),
                    '')
...
data.frame(customer=rep(c('김??????', '김??????'), each=2), 
...
paste(data[1:3, '???'], collapse='**')paste('aaa-bbb', 'ccc-ddd', sep='**')  
outer(month.abb, 2011:2020, paste, sep='-')
outer(LETTERS, 2010:2020, paste0)
grep(pattern='^2', x=data$??????, value = T)
...
data[1:3, '???']
paste(data[1:3, '???'])
paste(data[1:3, '???'], collapse='**')

outer(month.abb, 2011:2020, paste, sep='-')
outer(LETTERS, 2010:2020, paste0)

grep(pattern='^2.+0$', x=data$??????, value = T)
data$??????
...
head(data)
library(dplyr)
dplyr::rename(data, ?????? = math)
dplyr::rename(data, ?????? = kor)
library(ggplot2)
attach(data)
mean(??????)
...
with(data, mean(??????))
library(dplyr)
data = dplyr::rename(data,  math = ??????)
data %>% filter(group == 'C???')

data %>% filter(group == 'C???' & ?????? > 90)

data %>% filter(group %in% c('A???', 'C???'))


data %>% filter(math>95) %>% select(??????, ??????, ??????, math)
data %>%
  filter(math > 95) %>% 
  select(??????, ??????, ??????, math) %>%
  head
head(data)
data = dplyr::rename(data, art=??????)
data = dplyr::rename(data, kor=??????, sci=과학, eng=??????, art=??????)
data = dplyr::rename(data, stuno = ??????, cls = ???, gen = ??????)
...
ggplot(d2, aes(x=displ)) + 
  ...
  geom_bar(aes(fill=class),
           width = 0.5) +
  theme(axis.text.x = element_text(angle=45,       # 글?????? 기울???
                                   vjust=0.6)) +   # 글?????? ?????? 맞춤(?????????)
  ...
  ggplot(mpg, aes(cty)) +
    ...
    labs(title="밀???그래???", subtitle = "???린더?????? ?????? ?????????비의 밀???그래???",
  ...
  ggplot(d2, aes(x=displ)) + 
    ...
    xlab("x???") +
    xlim(1, 8) +
    scale_y_continuous("y???", limits = c(5, 45)) +
    labs(title = '?????????', subtitle = '?????? ?????????') 
...
kdata = kdata %>% rename(pop = 총인???_???)
kdata = kdata %>% rename(area = ??????구역???_???면동)
...
sqldf("select cls, (case when cls = '???' then '???리반' else '?????????' end) aa, 
...
txtSource = DirSource(folder)   # dir 경로??? Corpus ??????
...
crude = tm_map(crude, stripWhitespace)      # ?????? ???! (최종 ??????) 
...
#########################  분석???  Term & Document Matrix   #########################

# 분석??? Matrix ?????? (TermDocumentMatrix & DocumentTermMatrix)
tdm = TermDocumentMatrix(crude)   # ??????(???) * 문서(???)
tdm  
view(tdm)
rownames(tdm)      # ?????? 목록
...
tdm = removeSparseTerms(tdm, 0.8)  # ?????????(???????????????)??? 80% ????????? ?????? ??????
tdm; rownames(tdm)
dtm = t(tdm)                # ???치행???(Transpose)
inspect(tdm)                # ????????? 문서??? ?????? ???
inspect(tdm[1:5, 1:10])     # ??? 5??? ?????? 10??? 문서 (???번에 최??? 10개까지 가???)

######################  빈도 분석  ##################################################

# findFreqTerms()  & findAssocs()
findFreqTerms(tdm, 20)   # 20??? ?????? ????????? ??????
findFreqTerms(tdm, 20, 30)    # 20 ~ 30 ??? ????????? ?????? 
findFreqTerms(tdm, 0, 10)     # 10??? ?????? ?????? ??????
findAssocs(tdm, "last", 0.5)       # `last`??? ???관???(같이 ??????) 50% ??????
findAssocs(tdm, "oil", .7)         # `oil`??? ???관???(?????? ??????) 70% ??????

# rowSums(matrix)  ????????? 빈도??? 계산
rowSums(as.matrix(tdm))     
wFreq = sort(rowSums(as.matrix(tdm)), decreasing = T)     # 빈도??? ?????? ???
names(wFreq)
wFreq > 10


# subset() ?????? 조건?????? ????????????
wFreq = subset(wFreq, wFreq > 10)   # 빈도??? 10??? 초과 ????????????!
wFreq
...
display.brewer.all()            # brewer ?????? ??????
brewer.pal.info                 # brewer palette ??????
pa = brewer.pal(8, 'Blues')     # Blues Theme?????? 8가지 ?????? ??????
pa
