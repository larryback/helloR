...
autoparts <-read.csv("C:/workspace/R/helloR/data/공정기록데이터.csv",header = TRUE) #데이터 읽어 들이기
...
m12<- nnet(g_class ~ fix_time + a_speed + b_speed + separation+ 
             s_separation + rate_terms + mpa + load_time + highpressure_time, data = train,size = 12 ,abstol=1e-10,reltol=1e-10, maxit=100000 )
# nnet() 종속변수 ~ 독립변수 1 + 독립변수2 + ... , 데이터= 데이터 이름, 노드 갯수 = 숫자)

print(m12)  # 설계된 모델의 상세정보 확인

yhat_test <- predict(m12, test,type = "class") #설계된 모델을 활용하여 예측한 후 yhat_test로 저장
table<-table(real1=test$g_class,predict=yhat_test) # 교차 테이블 생성하기
table #테이블 확인
...
predict(m12, newdata = new.data,type = "class")
